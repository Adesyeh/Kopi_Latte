...
def tambah():
    conn = sqlite3.connect('cars515.db')
    c = conn.cursor()
    c.execute("""INSERT INTO cars515 (ID, BRAND, MODEL, PRICE) VALUES ("008", "Isuzu", "Carry", "50")""")
    conn.commit()
    c.close()
...
def tambah():
    conn = sqlite3.connect('cars515.db')
    c = conn.cursor()
    c.execute("""UPDATE cars515 SET MODEL='Carry Super' WHERE BRAND='Isuzu' """)
    conn.commit()
    c.close()
...
def tambah():
    conn = sqlite3.connect('cars515.db')
    c = conn.cursor()
    c.execute("""DELETE FROM cars515 WHERE ID='008'""")
    conn.commit()
    c.close()
...
def addNEW():
    ...
    if nid != "":        
        conn = sqlite3.connect('cars515.db')
        c = conn.cursor()
        c.execute("""INSERT INTO cars515 (ID, BRAND, MODEL, PRICE) VALUES (nid, nbrand, nmodel, nprice)""")
        conn.commit()
        c.close()
